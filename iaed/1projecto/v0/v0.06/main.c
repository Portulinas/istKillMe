...
double votos[NUM_PAISES][NUM_PARTIDOS][VOTACAO];  /* 2 arrays (Votos iniciais e quocientes)   */
...
void metodo(int EM){
	...
	while(deputados != 0){
		printf("Ciclo: %d\n",deputados);
		printf("Partido A: %f , Partido B: %f \n",votos[11][0][1],votos[11][0][1]);
		m = 0;
		for(i=0;i<NUM_PARTIDOS;i++){
		...
		printf("Partido A: %d , Partido B: %d \n",eleitos[11][0],eleitos[11][4]);
