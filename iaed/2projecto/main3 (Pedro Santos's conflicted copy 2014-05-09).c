...
void send(Queue Users) {
	...
	link last = Users[receiver].tail; /*ultimo elemento da fila*/
	link start = Users[receiver].head; /*cabeca da fila*/
	Item new_item;
	...
	if (isEmpty(Users, receiver)) {
		...
		insertEnd(last, new_item, new_node);
...
int main() {
	...
	scanf("%d", &N_USERS);
	Users  = malloc(N_USERS * sizeof(Queue));
