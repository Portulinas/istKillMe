...
Item newItem(char *message, int sender, int receiver) { 
		...
		new->e = sender;
		new->r = receiver;
		new->text = (char*)malloc((strlen(message)+1) * sizeof(char));
...
link insertEnd(Queue user, link tail, Item new_item, link new_node) {
	user.tail = new_node;
...
void send(Queue Users) {
	...
	if (isEmpty(Users, receiver)) {
		...
		last = insertEnd(Users[receiver], last, new_item, new_node);
